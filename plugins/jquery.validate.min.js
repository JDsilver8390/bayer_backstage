...
$.extend( $.validator, {
	...
	messages: {
		...
		rangelength: $.validator.format( "請輸入{0}-{1}個字元" ),
		range: $.validator.format( "請輸入介於{0}-{1}之間的值" ),
		max: $.validator.format( "請輸入一個小於或等於{0}的值" ),
		min: $.validator.format( "請輸入一個大於或等於{0}的值" ),
		step: $.validator.format( "請輸入 {0} 的倍數" ),
    IDcard:"身分證格式錯誤",
    IDCard:"不是合法身分證",
    twMobilePhone:"手機格式不正確",
    Landline:"電話格式不正確",
    Chinese: "請輸入中文",
    manageID:"請輸入8-12碼英數字",
    noChinese:"不可輸入中文",
    guiNB: "統一編號錯誤",
    guiNB2: "統一編號錯誤",
	},
...
//jquery.validate Custom rule by JOY ↓
...
$().ready(function() {
  ...
  //台灣手機
  $.validator.addMethod("twMobilePhone",function(value,element,params){
  var twMobilePhone = /^09\d{2}-?\d{3}-?\d{3}$/;
  return this.optional(element)||(twMobilePhone.test(value));
  });

  //台灣市話(含區碼)
  $.validator.addMethod("Landline",function(value,element,params){
  var Landline = /^0\d{1,2}-?\d{6,8}$/;
  return this.optional(element)||(Landline.test(value));
  });
  ...
  //管理員者帳號8-12碼英數字
  $.validator.addMethod("manageID",function(value,element,params){
  var manageID = /^\w{8,12}$/g;
  return this.optional(element)||(manageID.test(value));
  });

  //身分證字號+統一證號長度格式
  $.validator.addMethod("IDcard",function(value,element,params){
  var IDcard = /^[A-Z][1289]\d{8}$/;
  return this.optional(element)||(IDcard.test(value));
  });

  //身分證字號+統一證號是否合法
  $.validator.addMethod("IDCard",function(citizenid,element) {
  ...
  //統一編號(必填)
  $.validator.addMethod("guiNB",function(taxId,element) {
      ...
      sum = 0,
      calculate = function(product) { // Single digit + tens digit
      var ones = product % 10,
  ...
  //統一編號(非必填)
  $.validator.addMethod("guiNB2",function(taxId,element) {
    var a= this.value;
    if(document.getElementById("mem_cid").value==""){
      return true;
    }else{
      var invalidList = "00000000,11111111";
      if (/^\d{8}$/.test(taxId) == false || invalidList.indexOf(taxId) != -1) {
